...
// === PIControllerManagerStandardized.cpp - FIX DESTRUCTOR ===
...
// === PIControllerManagerStandardized.cpp - FIX DISCONNECT ALL ===

bool PIControllerManagerStandardized::DisconnectAll() {
	std::cout << "PIControllerManagerStandardized: DisconnectAll()" << std::endl;

	if (m_hardwareMode) {
		// Create a vector of devices to disconnect
		std::vector<std::unique_ptr<PIController>> devicesToDestroy;

		// Move all devices out of the map while holding the lock
		{
			std::lock_guard<std::mutex> lock(m_devicesMutex);
			devicesToDestroy.reserve(m_realDevices.size());

			for (auto& [name, device] : m_realDevices) {
				if (device) {
					std::cout << "  Preparing to disconnect: " << name << std::endl;
					devicesToDestroy.push_back(std::move(device));
				}
			}

			// Clear the map and update configs
			m_realDevices.clear();
			for (auto& [name, config] : m_deviceConfigs) {
				config.isConnected = false;
			}
		}

		// Now destroy all devices outside of any locks
		for (auto& device : devicesToDestroy) {
			if (device) {
				try {
					...
					std::cout << "  Exception during device destruction: " << e.what() << std::endl;
				}
			}
		}

		// Vector destructor will clean up devices safely
		std::cout << "  All PI devices destroyed" << std::endl;
	}
	else {
...
bool PIControllerManagerStandardized::ConnectDevice(const std::string& deviceName) {
	if (m_hardwareMode) {
		// Check if device is enabled before attempting connection
		...
		if (!config->isEnabled) {
			std::cout << "  Device is disabled in configuration: " << deviceName << std::endl;
			return false;
		}
...
// === PIControllerManagerStandardized.cpp - FIX DISCONNECT DEVICE METHOD ===

bool PIControllerManagerStandardized::DisconnectDevice(const std::string& deviceName) {
	if (m_hardwareMode) {
		std::unique_ptr<PIController> deviceToDestroy;

		// Move device out of map while holding lock briefly
		{
			std::lock_guard<std::mutex> lock(m_devicesMutex);
			auto it = m_realDevices.find(deviceName);
			if (it != m_realDevices.end()) {
				deviceToDestroy = std::move(it->second);
				m_realDevices.erase(it);

				// Update config
				auto configIt = m_deviceConfigs.find(deviceName);
				if (configIt != m_deviceConfigs.end()) {
					configIt->second.isConnected = false;
				}
			}
		}

		// Destroy device OUTSIDE of mutex lock
		if (deviceToDestroy) {
			std::cout << "  Disconnecting PI device: " << deviceName << std::endl;
			try {
				deviceToDestroy->StopAllAxes();
				std::this_thread::sleep_for(std::chrono::milliseconds(10));
				deviceToDestroy->Disconnect();
			...
			// deviceToDestroy goes out of scope here and destructor runs safely
			return true;
		}
		return false;
	}
...
bool PIControllerManagerStandardized::DisconnectDeviceUnsafe(const std::string& deviceName) {
	// Note: This method assumes the mutex is already locked or not needed (during destruction)
	auto it = m_realDevices.find(deviceName);
	if (it != m_realDevices.end()) {
		if (it->second) {
			try {
...
PIControllerManagerStandardized::PIDeviceConfig* PIControllerManagerStandardized::GetMutableDeviceConfig(const std::string& deviceName) {
	// Note: For use in contexts where mutex is already locked or during destruction
...
bool PIControllerManagerStandardized::GetDeviceIdentification(const std::string& deviceName, std::string& manufacturerInfo) {
	PIController* device = GetDevice(deviceName);
	if (!device) {
		std::cout << "PIControllerManagerStandardized: Device not found: " << deviceName << std::endl;
		return false;
	}

	return device->GetDeviceIdentification(manufacturerInfo);
}